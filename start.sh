...
NODE_APP="$ROOT_DIR/nginx/captive/start-server.sh"
HOSTAPD_DIR="$ROOT_DIR/nginx/configs"
INTERNET_IFACE="wlp3s0"  # Sesuaikan dengan interface internet Anda
AUTH_PAGE="~/home/rakasatryo/hostdirect/nginx/captive/default-captive/views/admin.ejs"    # Halaman autentikasi Anda
...
detect_hostapd_path() {
    local possible_paths=(
        "/home/rakasatryo/hostdirect/nginx/configs"
...
interface=$IFACE
dhcp-range=10.235.100.50,10.235.100.150,12h
dhcp-option=3,10.235.100.1
dhcp-option=6,10.235.100.1
bind-interfaces
# IMPORTANT: Prevent IPv6 and control DNS resolution
listen-address=10.235.100.1
no-resolv
filterwin2k
bogus-priv
stop-dns-rebind
...
address=/captive.apple.com/10.235.100.1
address=/captive.g.aaplimg.com/10.235.100.1
...
address=/#/10.235.100.1
# Additional settings for all devices
dhcp-option=252,"http://10.235.100.1"  # WPAD URL
...
server {
    listen 80;
    server_name portal.local 10.235.100.1 localhost;
    root /var/www/html;
    index $AUTH_PAGE;
    location / {
...
server {
    ...
    location / {
        add_header Cache-Control no-cache;
        add_header Content-Type text/html;
        return 200 "<html><head><script>window.location.href='http://10.235.100.1$AUTH_PAGE';</script></head><body><h2>Redirecting to WiFi Login...</h2><p>If you are not redirected automatically, <a href='http://10.235.100.1$AUTH_PAGE'>click here</a>.</p></body></html>";
    }
}
...
server {
    ...
    location / {
        proxy_pass http://10.235.100.1:3000;
...
server {
    ...
    location / {
        proxy_pass http://10.235.100.1:3002;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
...
server {
    ...
    location / {
        proxy_pass http://10.235.100.1:3001;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
...
server {
    ...
    location / {
        add_header Content-Type text/html;
        return 200 "<html><head><meta http-equiv='refresh' content='0; url=http://10.235.100.1$AUTH_PAGE'/></head><body>Redirecting...</body></html>";
    }
}
EOF
...
if [ $? -ne 0 ] || [ -z "$HOSTAPD_DIR" ]; then
    echo "[!] ❌ hostapd-mana directory not found"
    exit 1
else
...
cd "$HOSTAPD_DIR" || { echo "[!] Directory not found: $HOSTAPD_DIR"; exit 1; }

# Periksa apakah hostapd executable
if [ ! -x "./hostapd" ]; then
    echo "    ❌ hostapd executable not found or not executable in $HOSTAPD_DIR"
    exit 1
fi

# Jalankan hostapd di background dan catat log
echo "    → Starting hostapd with config: $HOSTAPD_CONF"
nohup ./hostapd $HOSTAPD_CONF > /var/log/hostapd-mana.log 2>&1 &
HOSTAPD_PID=$!
